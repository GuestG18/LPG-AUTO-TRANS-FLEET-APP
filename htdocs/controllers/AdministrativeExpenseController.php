...
{
    private AdministrativeExpenseModel $model;

    public function __construct(PDO $db)
    {
        $this->model = new AdministrativeExpenseModel($db);
    }

    public function handle(string $action): void
    {
        require_page_or_403('cheltuieli_administrative');
        ...
        switch ($action) {
            ...
            default:
                ...
                render('errors/404.php', [
                    ...
                    'currentPage' => 'cheltuieli_administrative',
                ]);
                return;
        }
    }
    ...
    private function indexAction(): void
    {
        ...
        try {
            ...
            $rows = $result['rows'];
            $viewData = [
                'pageTitle' => 'Cheltuieli Administrative',
                'currentPage' => 'cheltuieli_administrative',
                'subtitle' => 'Evidența cheltuielilor administrative care nu sunt asociate cu o locație',
                'dashboard' => $this->model->getDashboardData(),
                'rows' => $rows,
                'documentsByExpense' => $this->model->getDocumentsForRows($rows),
                'categories' => $this->model->getCategories(true),
                'filters' => $filters,
                'sort' => $sort,
                'direction' => $direction,
                'pagination' => [
                ...
                'paymentMethods' => AdministrativeExpenseModel::PAYMENT_METHODS,
                'documentTypes' => AdministrativeExpenseModel::DOCUMENT_TYPES,
            ];
        } catch (Throwable $exception) {
            error_log('[AdministrativeExpenseController][index] ' . $exception->getMessage());
            flash_set('danger', 'Modulul Cheltuieli Administrative necesita actualizarea bazei de date. Ruleaza database/update_cheltuieli_administrative.sql.');
            $viewData = [
                'pageTitle' => 'Cheltuieli Administrative',
                'currentPage' => 'cheltuieli_administrative',
                'subtitle' => 'Evidența cheltuielilor administrative care nu sunt asociate cu o locație',
        ...
        render('cheltuieli_administrative/index.php', $viewData);
    }
    ...
    private function exportAction(): void
    {
        ...
        header('Content-Disposition: attachment; filename="cheltuieli_administrative_' . date('Ymd_His') . '.csv"');
        echo "\xEF\xBB\xBF";
    ...
    private function storeAction(): void
    {
        ...
        try {
            ...
            flash_set('success', 'Cheltuiala administrativa a fost adăugată.');
        } catch (Throwable $exception) {
            if ($documentData !== null) {
                $this->deletePhysicalDocument((string) ($documentData['stored_name'] ?? ''));
            }
            error_log('[AdministrativeExpenseController][store] ' . $exception->getMessage());
    ...
    private function updateAction(): void
    {
        ...
        try {
            ...
            error_log('[AdministrativeExpenseController][update] ' . $exception->getMessage());
    ...
    private function deleteAction(): void
    {
        ...
        try {
            ...
            error_log('[AdministrativeExpenseController][delete] ' . $exception->getMessage());
    ...
    private function validateExpenseInput(array $data): array
    {
        ...
        if (!array_key_exists((string) ($data['payment_method'] ?? ''), AdministrativeExpenseModel::PAYMENT_METHODS)) {
    ...
    private function storeUploadedDocument(?array $file, string $documentType): array
    {
        ...
        $documentType = array_key_exists($documentType, AdministrativeExpenseModel::DOCUMENT_TYPES) ? $documentType : 'factura';
        $maxSize = 5242880;
        ...
        try {
            $storedName = 'admin_expense_' . date('YmdHis') . '_' . bin2hex(random_bytes(8)) . '.' . $extension;
        } catch (Throwable) {
            $storedName = 'admin_expense_' . date('YmdHis') . '_' . str_replace('.', '', uniqid('', true)) . '.' . $extension;
        }
    ...
    {
        return AdministrativeExpenseModel::PAYMENT_METHODS[$method] ?? $method;
    }
    ...
    {
        return build_query_url(['page' => 'cheltuieli_administrative']);
    }
}
