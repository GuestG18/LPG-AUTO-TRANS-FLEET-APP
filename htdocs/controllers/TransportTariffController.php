...
class TransportTariffController
{
    ...
    public function handle(string $action): void
    {
        switch ($action) {
            ...
            case 'delete_version':
                ...
                return;
            case 'store_versions_bulk':
                $this->storeVersionsBulkAction();
                return;
            case 'preview':
                $this->previewAction();
                return;
    ...
    private function storeVersionAction(): void
    {
        ...
        if ($validTo !== '' && (preg_match('/^\d{4}-\d{2}-\d{2}$/', $validTo) !== 1 || !$this->isValidDate($validTo) || $validTo < $validFrom)) {
            $errors[] = 'Data „Valabil până la" este invalidă (trebuie să fie cel puțin egală cu data de start).';
        }
        ...
        try {
            ...
            $this->model->recordHistory([
                ...
                'effective_to' => $validTo !== '' ? $validTo : null,
                'reference_fuel_price' => $reference['price'],
            ...
            if (!empty($created['continuation_id'])) {
                $this->model->recordHistory([
                    'tariff_version_id' => $created['continuation_id'],
                    'rule_signature' => TransportTariffModel::buildSignature($beneficiaryId, $componentKey, $routeRefId > 0 ? $routeRefId : null),
                    'beneficiar_id' => $beneficiaryId,
                    'transport_type' => $transportType,
                    'component_key' => $componentKey,
                    ...
                    'action' => 'scheduled',
                    'old_value' => $value,
                    'new_value' => $created['previous_value'],
                    'unit' => TransportTariffModel::componentUnit($componentKey),
                    'effective_from' => (string) $created['continuation_from'],
                    'effective_to' => $created['continuation_to'],
                    'reason' => 'Continuare automată a tarifului anterior după intervalul închis.',
                    'changed_by' => $this->currentUserId(),
                    'changed_by_name' => $this->currentUserName(),
                    'changed_at' => date('Y-m-d H:i:s'),
                ]);
            ...
            $message = $validFrom > date('Y-m-d')
                ? sprintf('Tarif programat: %s de la %s', format_number_ro($value, 4), $this->formatDateRo($validFrom))
                : sprintf('Tarif actualizat: %s de la %s', format_number_ro($value, 4), $this->formatDateRo($validFrom));
            if ($validTo !== '') {
                $message .= ' până la ' . $this->formatDateRo($validTo);
            }
            $message .= '.';
            if (!empty($created['replaced_version_id'])) {
                $message .= ' Versiunea cu aceeași dată de start a fost înlocuită.';
            }
            if (!empty($created['continuation_id'])) {
                $message .= sprintf(
                    ' După %s tariful anterior (%s) redevine activ automat.',
                    $this->formatDateRo($validTo),
                    format_number_ro((float) $created['previous_value'], 4)
                );
            }
    ...
    /**
     * Save several tariff components at once (e.g. all Compresor rates, or a
     * route's Tonă + Km + Cost/cursă) with a single effective date. Blank
     * inputs are skipped. All-or-nothing: one transaction, one history entry
     * per component, then the standard reprice preview covers them together.
     */
    private function storeVersionsBulkAction(): void
    {
        $this->requireManage();
        ...
        // --- collect the filled rows -----------------------------------
        $changes = [];
        foreach ($componentKeys as $index => $componentKey) {
            $componentKey = trim((string) $componentKey);
            $rawValue = trim((string) ($rawValues[$index] ?? ''));
            if ($rawValue === '') {
                continue; // blank = keep unchanged
            }

            $componentMeta = TransportTariffModel::COMPONENTS[$componentKey] ?? null;
            if ($componentMeta === null) {
                $errors[] = 'Componentă tarifară invalidă: ' . $componentKey . '.';
                continue;
            }

            $value = $this->normalizeDecimal($rawValue);
            if ($value === null || $value < 0) {
                $errors[] = 'Valoare invalidă pentru "' . TransportTariffModel::componentLabel($componentKey) . '" (număr ≥ 0).';
                continue;
            }
        ...
        if ($changes === [] && $errors === []) {
            $errors[] = 'Nu ai completat nicio valoare — nu s-a modificat nimic.';
        }
        if ($errors !== []) {
            flash_set('danger', implode(' ', $errors));
            redirect(build_query_url($redirect));
        ...
        try {
            ...
            foreach ($changes as $change) {
                $created = $this->model->createVersion([
                    ...
                    'route_scope' => $change['route_scope'],
                    'route_ref_id' => $change['route_ref_id'] > 0 ? $change['route_ref_id'] : null,
                    'loc_incarcare_id' => $change['loc_id'],
                    'zona_distributie_id' => $change['zone_id'],
                    'value' => $change['value'],
                    'valid_from' => $validFrom,
                    'valid_to' => $validTo !== '' ? $validTo : null,
                    'fuel_weight' => null,
                ...
                $this->model->recordHistory([
                    'tariff_version_id' => $created['version_id'],
                    'rule_signature' => TransportTariffModel::buildSignature($beneficiaryId, $change['component_key'], $change['route_ref_id'] > 0 ? $change['route_ref_id'] : null),
                    'beneficiar_id' => $beneficiaryId,
                    'transport_type' => $transportType,
                    'component_key' => $change['component_key'],
                    'route_ref_id' => $change['route_ref_id'] > 0 ? $change['route_ref_id'] : null,
                    'route_label' => $this->buildRouteLabel($change['loc_id'], $change['zone_id']),
                    'action' => $validFrom > date('Y-m-d') ? 'scheduled' : 'created',
                    'old_value' => $created['previous_value'],
                    'new_value' => $change['value'],
                    'unit' => TransportTariffModel::componentUnit($change['component_key']),
                    'effective_from' => $validFrom,
                    'effective_to' => $validTo !== '' ? $validTo : null,
                    'reference_fuel_price' => $reference['price'],
                    'observed_fuel_price' => $monitoring['current_price'] ?? null,
                ...
                $createdIds[] = (int) $created['version_id'];
            }
            ...
            error_log('[TransportTariffController][store_versions_bulk] ' . $exception->getMessage());
            flash_set('danger', 'Nu s-a salvat niciun tarif: ' . $exception->getMessage());
        }
